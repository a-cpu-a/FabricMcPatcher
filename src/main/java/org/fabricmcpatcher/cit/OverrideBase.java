...
abstract class OverrideBase implements Comparable<OverrideBase> {
    ...
    final Map<String, Identifier> alternateTextures;
    final int weight;
    ...
    static OverrideBase create(Identifier filename) {
    ...
    private Map<String, Identifier> getAlternateTextures() {
        Map<String, Identifier> tmpMap = new HashMap<String, Identifier>();
        for (Map.Entry<String, String> entry : properties.entrySet()) {
            ...
            Identifier resource = TileLoader.parseTileAddress(properties.getResource(), value);
            if (resource != null) {
    ...
    private boolean matchDamage(ItemStack itemStack) {
        return damage == null || damage.get(itemStack.getDamage() & damageMask);
    }
    ...
    private boolean matchDamagePercent(ItemStack itemStack) {
        ...
        int percent = (100 * itemStack.getDamage()) / maxDamage;
        if (percent < 0) {
            percent = 0;
        } else if (percent > 100) {
            percent = 100;
    ...
    private boolean matchStackSize(ItemStack itemStack) {
        return stackSize == null || stackSize.get(itemStack.getCount());
    }
