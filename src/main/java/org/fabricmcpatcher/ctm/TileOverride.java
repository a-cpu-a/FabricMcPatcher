...
abstract class TileOverride implements ITileOverride {
    ...
    private final List<Identifier> tileNames = new ArrayList<>();
    protected Sprite[] icons;
    private int matchMetadata = META_MASK;

    static TileOverride create(Identifier propertiesFile, TileLoader tileLoader) {
    ...
    private void loadIcons() {
        ...
        Identifier blankResource = RenderPassAPI.instance.getBlankResource(renderPass);
        if (tileList.equals("")) {
            for (int i = 0; ; i++) {
                Identifier resource = TileLoader.parseTileAddress(properties.getResource(), String.valueOf(i), blankResource);
            ...
            for (String token : tileList.split("\\s+")) {
                ...
                if (token.equals("")) {
                    ...
                    Identifier resource = TileLoader.parseTileAddress(properties.getResource(), token, blankResource);
                    if (resource == null) {
                        tileNames.add(null);
    ...
    private Set<String> getTileList(String key) {
        ...
        for (String token : property.split("\\s+")) {
            if (token.equals("")) {
                ...
                Identifier resource = TexturePackAPI.parseIdentifier(properties.getResource(), token);
                if (resource != null) {
    ...
    public final void registerIcons() {
        icons = new Sprite[tileNames.size()];
    ...
    final boolean shouldConnect(RenderBlockState renderBlockState, Sprite icon, int relativeDirection) {
        return shouldConnect(renderBlockState, icon, renderBlockState.getOffset(renderBlockState.getBlockFace(), relativeDirection));
    }

    final boolean shouldConnect(RenderBlockState renderBlockState, Sprite icon, int blockFace, int relativeDirection) {
        return shouldConnect(renderBlockState, icon, renderBlockState.getOffset(blockFace, relativeDirection));
    }

    private boolean shouldConnect(RenderBlockState renderBlockState, Sprite icon, int[] offset) {
        BlockRenderView blockAccess = renderBlockState.getBlockAccess();
        ...
        switch (connectType) {
            ...
            case CONNECT_BY_MATERIAL:
                //return block.blockMaterial == neighbor.blockMaterial; //CHANGED!
                return block.getDefaultMapColor() == neighbor.getDefaultMapColor();
    ...
    @Override
    public final Sprite getTileWorld(RenderBlockState renderBlockState, Sprite origIcon) {
        if (icons == null) {
    ...
    @Override
    public final Sprite getTileHeld(RenderBlockState renderBlockState, Sprite origIcon) {
        if (icons == null) {
            properties.error("no images loaded, disabling");
            return null;
        }
    ...
    abstract Sprite getTileWorld_Impl(RenderBlockState renderBlockState, Sprite origIcon);

    abstract Sprite getTileHeld_Impl(RenderBlockState renderBlockState, Sprite origIcon);
}