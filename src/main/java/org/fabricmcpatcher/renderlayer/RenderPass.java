...
import org.fabricmcpatcher.ctm.CTMUtils;
...
public class RenderPass {
    ...
    private static Identifier blendBlankResource;
    ...
    static {
        RenderPassAPI.instance = new RenderPassAPI() {
            ...
            public void refreshBlendingOptions() {
                PropertiesFile properties = PropertiesFile.get(logger, TexturePackAPI.newMCPatcherIdentifier(RENDERPASS_PROPERTIES));
                if (properties != null) {
            ...
            @Override
            public Identifier getBlankResource(int pass) {
    ...
    // 14w02a+
    public static boolean shouldSideBeRendered(Block block, BlockRenderView blockAccess, BlockPos position, Direction direction) {
        if (BlockAPI.shouldSideBeRendered(block,blockAccess, position, direction)) {
            return true;
        } else if (!extraRenderPass.containsKey(block)) {
            Block neighbor = blockAccess.getBlockState(position).getBlock();
            return extraRenderPass.containsKey(neighbor);
        } else {
            return false;
        }
